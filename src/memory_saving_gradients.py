...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
    ...
    bwd_ops = tf.contrib.graph_editor.get_backward_walk_ops([y.op for y in ys],
                                       inclusive=True)

    debug_print("bwd_ops: %s", bwd_ops)

    # forward ops are all ops that are candidates for recomputation
    fwd_ops = tf.contrib.graph_editor.get_forward_walk_ops([x.op for x in xs],
                                      inclusive=True,
                                      within_ops=bwd_ops)
    ...
    ts_all = tf.contrib.graph_editor.filter_ts(fwd_ops, True) # get the tensors
    ...
    if type(checkpoints) is not list:
        if checkpoints == 'collection':
            ...
            checkpoints = tf.contrib.graph_editor.filter_ts_from_regex(fwd_ops, 'conv2d|Conv|MatMul')
            ...
            for ts in [ts_filtered, ts_all]:
                ...
                bottleneck_ts = []
                for t in ts:
                    b = set(tf.contrib.graph_editor.get_backward_walk_ops(t.op, inclusive=True, within_ops=fwd_ops))
                    f = set(tf.contrib.graph_editor.get_forward_walk_ops(t.op, inclusive=False, within_ops=fwd_ops))
    ...
    copied_sgv, info = tf.contrib.graph_editor.copy_with_input_replacements(tf.contrib.graph_editor.sgv(ops_to_copy), {})
    for origin_op, op in info._transformed_ops.items():
        op._set_device(origin_op.node_def.device)
    copied_ops = info._transformed_ops.values()
    debug_print("Copied %s to %s", ops_to_copy, copied_ops)
    tf.contrib.graph_editor.reroute_ts(checkpoints_disconnected.values(), checkpoints_disconnected.keys(), can_modify=copied_ops)
    ...
    for ts in checkpoints_sorted_lists[::-1]:
        ...
        copied_ops = info._transformed_ops.values()
        debug_print("Copied %s to %s", ops_to_copy, copied_ops)
        tf.contrib.graph_editor.reroute_ts(checkpoints_disconnected_other, checkpoints_other, can_modify=copied_ops)
        debug_print("Rewired %s in place of %s restricted to %s",
...
def tf_toposort(ts, within_ops=None):
    all_ops = tf.contrib.graph_editor.get_forward_walk_ops([x.op for x in ts], within_ops=within_ops)
...
def fast_backward_ops(within_ops, seed_ops, stop_at_ts):
    bwd_ops = set(tf.contrib.graph_editor.get_backward_walk_ops(seed_ops, stop_at_ts=stop_at_ts))
...
def capture_ops():
  ...
  op_list.extend(tf.contrib.graph_editor.select_ops(scope_name+"/.*", graph=g))
...
def my_add_control_inputs(wait_to_do_ops, inputs_to_do_before):
    for op in wait_to_do_ops:
        ...
        tf.contrib.graph_editor.add_control_inputs(op, ci)
